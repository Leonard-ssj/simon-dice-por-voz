...
void testD_vocabulario() {
  ...
  // Demo: reproduce una seleccion representativa del vocabulario
  struct { VozID id; const char* label; } demo[] = {
    { VOZ_SRV_LISTO,      "srv_listo"      },
    { VOZ_MIRA_ESCUCHA,   "mira_escucha"   },
    { VOZ_COLOR_ROJO,     "color_rojo"     },
    { VOZ_COLOR_VERDE,    "color_verde"    },
    { VOZ_COLOR_AZUL,     "color_azul"     },
    { VOZ_COLOR_AMARILLO, "color_amarillo" },
    { VOZ_TURNO,          "turno"          },
    { VOZ_CORRECTO,       "correcto"       },
    { VOZ_INCORRECTO,     "incorrecto"     },
    { vozNivel(5),        "nivel_05"       },
    { vozCorrectos(3),    "correctos_03"   },
    { vozFin(7),          "fin_07"         },
  };
...
void setup() {
  ...
  // Montar LittleFS (no fatal si falla — solo la opcion D no funciona)
  if (LittleFS.begin(false)) {
    Serial.printf("LittleFS OK (%d KB usados / %d KB total)\n",
                  (int)(LittleFS.usedBytes() / 1024),
                  (int)(LittleFS.totalBytes() / 1024));
  } else {
    Serial.println("LittleFS: no montado (sube archivos con LittleFS Data Upload)");
  }
